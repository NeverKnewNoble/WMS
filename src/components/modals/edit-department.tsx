...
import { Building, Hash, Pencil, X } from "lucide-react";
...
export default function EditDepartmentDialog({
  ...
  if (!department || !mounted) return null;
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      showSuccessToast("Department updated", `${get("label") || department.label} saved.`);
      onSaved?.();
      onClose();
    } catch {
      // toast already shown
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-white/50">
                  Update details for{" "}
                  <span className="font-medium text-white/80">{department.label}</span>.
                </p>
              </div>
            </div>
            <button
