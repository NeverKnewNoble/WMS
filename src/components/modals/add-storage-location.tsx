...
import { Plus, X, Hash, Warehouse, MapPin } from "lucide-react";
import { FieldLabel, fieldClass } from "../ui_components/portal/primitives";
import { createStorageLocation } from "@/services/storage-locations";
import { showSuccessToast } from "@/services/toast";

export default function AddStorageLocationDialog({
  onCreated,
}: {
  onCreated?: () => void;
}) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      showSuccessToast("Storage location created", `${loc.label} added.`);
      onCreated?.();
      close();
    } catch {
      // toast already shown
                ...
                <div className="flex shrink-0 items-start justify-between gap-4 border-b border-white/8 px-6 pb-5 pt-6">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-white/50">
                      Warehouses, yards, or sub-stores where stock is physically held.
                    </p>
                  </div>
                  <button
                    type="button"
